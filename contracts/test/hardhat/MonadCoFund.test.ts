...
describe("MonadCoFund", function () {
  ...
  describe("1. Fund Creation", function () {
    it("should create a fund successfully", async function () {
      const tx = await cofund.connect(alice).createFund("Goa Trip", "Shared expenses", parseEther("100"), 2);
      await tx.wait();

      const f = await cofund.getFund(1);
      expect(f.name).to.equal("Goa Trip");
      expect(f.purpose).to.equal("Shared expenses");
      expect(f.target).to.equal(parseEther("100"));
    ...
    it("should make creator the first member", async function () {
      await cofund.connect(alice).createFund("Goa Trip", "Shared expenses", parseEther("100"), 2);
    ...
    it("should increment fund IDs", async function () {
      await cofund.connect(alice).createFund("Fund 1", "Purpose 1", parseEther("10"), 1);
      await cofund.connect(bob).createFund("Fund 2", "Purpose 2", parseEther("20"), 1);
  ...
  describe("2. Membership", function () {
    ...
    it("should prevent non-members from depositing", async function () {
      await expect(
        cofund.connect(eve).deposit(1, { value: parseEther("1") })
      ).to.be.revertedWithCustomError(cofund, "NotMember");
    });

    it("should prevent non-members from creating proposals", async function () {
      await cofund.connect(alice).createCategory(1, "Villa", parseEther("40"));
      await expect(
        cofund.connect(eve).createProposal(1, 1, bob.address, parseEther("10"), "Rent")
      ).to.be.revertedWithCustomError(cofund, "NotMember");
    });

    it("should prevent non-members from approving proposals", async function () {
      await cofund.connect(alice).createCategory(1, "Villa", parseEther("40"));
      await cofund.connect(alice).createProposal(1, 1, bob.address, parseEther("10"), "Rent");
      await expect(
  ...
  describe("3. Treasury & Deposits", function () {
    ...
    it("should accept valid deposits and update balances", async function () {
      await cofund.connect(alice).deposit(1, { value: parseEther("10") });
      await cofund.connect(bob).deposit(1, { value: parseEther("5") });

      const f = await cofund.getFund(1);
      expect(f.balance).to.equal(parseEther("15"));
      expect(await cofund.getContribution(1, alice.address)).to.equal(parseEther("10"));
      expect(await cofund.getContribution(1, bob.address)).to.equal(parseEther("5"));
    });
    ...
    it("should isolate funds between different fund IDs", async function () {
      ...
      await cofund.connect(alice).deposit(1, { value: parseEther("10") });
      await cofund.connect(bob).deposit(2, { value: parseEther("5") });

      expect((await cofund.getFund(1)).balance).to.equal(parseEther("10"));
      expect((await cofund.getFund(2)).balance).to.equal(parseEther("5"));
    });
  });
  ...
  describe("4. Categories / Budgets", function () {
    ...
    it("should revert if budget is 0 or name is empty", async function () {
      ...
      await expect(
        cofund.connect(alice).createCategory(1, "", parseEther("40"))
      ).to.be.revertedWithCustomError(cofund, "EmptyString");
    });
  });
  ...
  describe("5. Proposals, Approvals & Execution", function () {
    beforeEach(async function () {
      ...
      await cofund.connect(alice).createCategory(1, "Villa", parseEther("40"));
      await cofund.connect(alice).deposit(1, { value: parseEther("50") });
    });

    it("should create a proposal and track state", async function () {
      await cofund.connect(alice).createProposal(1, 1, dave.address, parseEther("10"), "Villa deposit");
      const p = await cofund.getProposal(1, 1);
      expect(p.purpose).to.equal("Villa deposit");
      expect(p.amount).to.equal(parseEther("10"));
    ...
    it("should revert execution if category budget is exceeded", async function () {
      await cofund.connect(alice).createProposal(1, 1, dave.address, parseEther("45"), "Over budget");
      await cofund.connect(alice).approveProposal(1, 1);
      await cofund.connect(bob).approveProposal(1, 1);
    ...
    it("should execute payment correctly, transfer MON, and update treasury & category spent", async function () {
      await cofund.connect(alice).createProposal(1, 1, dave.address, parseEther("30"), "Villa booking");
      await cofund.connect(alice).approveProposal(1, 1);
      await cofund.connect(bob).approveProposal(1, 1);
      ...
      expect(daveBalanceAfter - daveBalanceBefore).to.equal(parseEther("30"));
      expect((await cofund.getFund(1)).balance).to.equal(parseEther("20"));
      expect((await cofund.getCategory(1, 1)).spent).to.equal(parseEther("30"));
